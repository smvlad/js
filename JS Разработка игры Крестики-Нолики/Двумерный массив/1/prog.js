...
function check2Diag() {
	if (refs2[0][0].innerHTML =="" && refs2[refs2.length-1][0].innerHTML =="")      //если пусто, то точно не победа и выходим.  
		return false;
	
	// Главная диагональ
	if (refs2[0][0].innerHTML =="")
		res1 = false;
	else
		res1 = true; 
	for (var i = 0; i < refs2.length-1; i++) {
		if (refs2[i][i].innerHTML!=refs2[i+1][i+1].innerHTML)
			res1 = false;
	}

	// Вспомогательная диагональ
	if (refs2[refs2.length-1][0].innerHTML =="")
		res2 = false;
	else
		res2 = true; 
	for (var i = 0; i < refs2.length-1; i++) {
		if (refs2[refs2.length-1-i][i].innerHTML!=refs2[refs2.length-2-i][i+1].innerHTML)
			res2 = false;
	}
	...
	// 	res = true;
	return res1 || res2;   // проверяет диагонали. Вызывается в  win
}

function checkHorVert() {
	res = false;
	for (var i = 0; i < refs2.length; i++) {   // цикл по строкам
...
function OneHor(nomerHor) {   //  Вызывается в checkHorVert  Идет по нужной строке и сравнивает соседние
	if (refs2[nomerHor][0].innerHTML== "")
		return false;
	res=true;
	for (var i = 0; i < refs2.length-1; i++) {
		if (refs2[nomerHor][i].innerHTML!=refs2[nomerHor][i+1].innerHTML)
			res = false;
	}
	return res; 
}
...
function OneVert(nomerVert) {   //  Вызывается в check2Hor 1 -ый столбец: индексы ячеек  0,3,6  // 2-ой столбец: 1,4,7
	...
	res=true;
	for (var i = 0; i < refs2.length-1; i++) {
		if (refs2[i][nomerVert].innerHTML!=refs2[i+1][nomerVert].innerHTML)
			res = false;
	}
	return res; 
