...
var ref_comp_addr = document.querySelector('#dec');
...
function f1(argument) {
	...
	if (hostaddr == Math.pow(2, 32-num1_in_mask)-1)
		...
		ref_comp_addr.innerText = hostaddr;

	str_bin = '';
	para16 = '';
	hostaddr16_str = hostaddr.toString(16);  //преобразование адреса компа из десятичного в шестнадцатеричный
	clear_old_addr(ref_inputs);                        //очистка старого адреса
...
function clear_old_addr(ref_inputs) {   //адрес компа ref_inputs[k] - ref_inputs[l], где k =12, l = 15
	for (var i = 12; i < 16; i++) {
		ref_inputs[i].value = ""
	}
}
