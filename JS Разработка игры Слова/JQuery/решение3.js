...
window.onload = function(){
  ...
  btnStart.on('click', getNewWord);
  txtResult.on('keyup', checkInput);
...
function changeTimer(){
  strTimer[0].textContent = secondsLeft + ' секунд' + end(secondsLeft);
}
...
function getNewWord(){
  ...
  strWord[0].textContent = currentWord.split("").shuffle().join("");   // перемешиваем буквы в слове
  
  txtResult[0].value = '';
  txtResult.focus();
  btnStart[0].disabled = 'disabled';
...
function checkInput(){
  if(txtResult[0].value.toUpperCase() == currentWord){
    strWin.textContent = ++numWin;
    clearInterval(timer);
    btnStart[0].disabled = '';
    btnStart.focus();
