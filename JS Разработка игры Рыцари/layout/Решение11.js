...
//screenHsize=screen.availHeight     доступная высота окна. Не включает таскбар и верхнюю панель.
let speedplayer=7, numAnimImgs = 5, bumpDistance=80, timetodie=1000, bornline=screen.availWidth-200, 
screenHsize=screen.availHeight, orcheight = 100, orcwidth=100, playerheight=100, playerwidth=100, polepos,
screenWsize=screen.availWidth, fonWidth, fonMoveSize,
kuvaldabump=false;
//fonMoveSize величина, на которую фон шире окна. И на которую, соответственно, мы его можем двигать
let pl1=null, игра=null, orc1=[],orc2=[],orc3=[],secondspassed=0,numtype1 = 5, numtype2 = 3,numtype3 = 2;
...
let refmpid = $('#mpid');
let polejbject=document.getElementById('gamepole');
var startpanel=document.getElementsByClassName('panel');

var pausegame=false;

middle = Math.floor(screenWsize/2);
/****************  начальная позиция фона и его размер   **************************************************/
polepos=0;
fonMoveSize = Math.floor(screenWsize/5);    // ~20%
fonWidth=screenWsize+fonMoveSize;
polejbject.style['background-size'] ="120% 100%";
/****************  начальная позиция фона и его размер   **************************************************/

refformid.addEventListener('submit', checkname);
// refplayername.addEventListener('keyup', checkname);
refplayername.addEventListener('input', checkname);          //input в данном случае правильнее, поскольку мы не собираемся отсеивать какие-то нажатия и не выводить их на экран
...
class player {
	...
	constructor (top, left, imagePath,imageName,imageExt) {
		...
		this.polejbject = polejbject;
		this.polejbject.appendChild(this.imgobject);
/*********************** формирование координаты объекта из строки сдвига  в пикселях ***********************************************************/ 
		массивСимволов = left.split("");					//преобразуем строку в массив символов
		массивСимволов.length-=2;  						    //отрезаем два последних символа          - это "px"
...
/*********************** формирование координаты объекта из строки сдвига  в пикселях ***********************************************************/
		...
		this.imgobject.style["left"]=left;           // начальный вывод игрока в место, указанное в параметрах
		this.imgobject.style["top"]=top;			// начальный вывод игрока в место, указанное в параметрах
	}
	...
	moveDown() {
		let prefiximg = "RUN";
		if (this.top<screenHsize+this.speed-240) {
			this.top+=this.speed;
			this.imgobject.style["top"]=this.top+"px";
			this.changeimage(prefiximg);
		}
	...
	update() {
		if (!pausegame) {
			this.status++;
			if (this.status%2 == 0 && kuvaldabump) {
...
class orc {
	constructor (top, left, imagePath,imageName,imageExt,type) {
		...
		this.height=orcheight;
		this.width = orcwidth;
		this.imgobject.style.width=this.width+"px";
		this.imgobject.style.height=this.height+"px";
		
		this.poleobject = polejbject;
...
/////////////  получаем числовое значение координат
	...
	checkBump() {   // проверяем столкновение
		let leftdif = this.calcdistance(1);  // горизонтальное расстояние до игрока
		let kvadrat1 = Math.pow(leftdif,2);
		
		let topdif = this.calcdistance(2);   // вертикальное расстояние
...
class game {
	constructor () {
		let location, bottomshift = 210;
		pl1 = new player("200px","10px", "../media/Орки/_PNG/1_ORK/","_000",".png");

		for (var i = 0; i < numtype1; i++) {
			location=getRandomInt(0, screenHsize-bottomshift)+'px';
			orc1[i] = new orc(location,bornline+"px","../media/Рыцари/_PNG/1/","3_RUN_000",".png",1);
		}
		for (var i = 0; i < numtype2; i++) {
			location=getRandomInt(0, screenHsize-bottomshift)+'px';
			orc2[i] = new orc(location,bornline+"px","../media/Рыцари/_PNG/2/","3_RUN_000",".png",2);
		}
		for (var i = 0; i < numtype3; i++) {
			location=getRandomInt(0, screenHsize-bottomshift)+'px';
			orc3[i] = new orc(location,bornline+"px","../media/Рыцари/_PNG/3/","3_RUN_000",".png",3);
		}
    ...
    stop () {
    	pausegame=true;
		startpanel[1].style.visibility="visible";
    }
}
...
function next() {    //убираем форму и стартуем игру
	let coords;
	for (var i = 0; i < startpanel.length; i++) {
		startpanel[i].style.visibility="hidden";
	}
	...
	// document.onkeypress=keyManage;    //не работает    
	// document.onkeydown=keyManage;    // работает    
	// document.onkeyup=keyManage;    // работает только приотпускании
	    // document.onkeydown=document.onkeyup=keyManage;   ломается ESC - остановка игры
...
function keyManage(e) {
	switch(e.keyCode) {
		...
		case 39:		//стрелка вправо
			if (pl1.left>=middle && Math.abs(polepos)<=fonMoveSize)
				fonmoveleft();
			else
				pl1.moveRight();
			break;
...
function fonmoveleft() {
	//let ref_screen_game=document.getElementsByClassName('.screen-game');   //неудача. html-коллекция нулевой длины. Видимо потому, что у нас нет ни одного объекта только с этим классом.
			polepos -=speedplayer;
			polejbject.style['background-position']=polepos+"px"+" 0px";
}
// next();
